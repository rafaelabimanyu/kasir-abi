...
<body class="antialiased min-h-screen overflow-x-hidden" x-data="globalShortcuts()">
    <div x-data="{ sidebarOpen: false }" x-init="$watch('sidebarOpen', val => { if (window.innerWidth < 1024) document.body.style.overflow = val ? 'hidden' : '' })"
            ...
            <footer class="px-6 py-4 border-t border-dark-700/50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-2 text-sm text-slate-500">
                    <div class="flex items-center flex-wrap gap-2">
                        <p>&copy; {{ date('Y') }} {{ \App\Models\Setting::get('store_name', 'Kasir Abi') }}. All
                            rights reserved.</p>
                        <span class="hidden sm:inline-block mx-1">|</span>
                        <button @click="showShortcutModal = true"
                            class="hover:text-white transition-colors flex items-center gap-1.5 focus:outline-none cursor-pointer">
    ...
    <div x-show="showShortcutModal" x-transition.opacity.duration.300ms
        class="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4"
        style="display: none;">
        <div @click.away="showShortcutModal = false" x-show="showShortcutModal"
            x-transition.scale.origin.bottom.duration.300ms
            class="bg-dark-800 border border-dark-600 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-full">
            <div class="flex items-center justify-between p-5 border-b border-dark-600/50">
                ...
                <button @click="showShortcutModal = false"
                    class="text-slate-400 hover:text-white transition-colors cursor-pointer focus:outline-none">
                    <i data-lucide="x" class="w-5 h-5"></i>
                </button>
            </div>
            <div class="p-6 overflow-y-auto space-y-6">
                <div>
                    <h4 class="text-sm font-semibold text-slate-300 mb-3 uppercase tracking-wider">Global (Semua
                        Halaman)</h4>
                    <div class="space-y-2">
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Buka / Tutup
                                Daftar Shortcut</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">Ctrl
                                + /</kbd></div>
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Fokus Global
                                Search</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">Ctrl
                                + K</kbd></div>
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Ke Halaman
                                Dashboard</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">Ctrl
                                + H</kbd></div>
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Ke Halaman
                        ...
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Ke Halaman
                                ...
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">Ctrl
                                + L</kbd></div>
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Ke User
                                Management</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">Ctrl
                                + U</kbd></div>
                    </div>
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Fokus
                                Pencarian Produk</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">F2</kbd>
                        </div>
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Proses
                                Pembayaran</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">Enter</kbd>
                        </div>
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Reset / Void
                                Keranjang</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">ESC</kbd>
                        </div>
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Simpan
                                Transaksi (Paksa)</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">Ctrl
                                + Shift + S</kbd></div>
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Hold
                                Transaksi</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">Ctrl
                                + Shift + D</kbd></div>
                        <div class="flex justify-between items-center text-sm"><span class="text-slate-400">Kosongkan
                                Keranjang</span><kbd
                                class="px-2 py-1 bg-dark-600 text-slate-300 rounded border border-dark-500 font-mono text-xs shadow-sm">Ctrl
                                + Backspace</kbd></div>
                    </div>
            ...
            <div class="p-4 border-t border-dark-600/50 bg-dark-800/80 text-center">
                <a href="{{ url('shortcuts') }}"
                    class="text-brand-400 hover:text-brand-300 text-sm font-medium transition-colors">Lihat Panduan
                    Lengkap &rarr;</a>
            </div>
        </div>
    </div>
    ...
    <div x-data="globalToast()" @notify.window="showToast($event.detail.message, $event.detail.type)"
        class="fixed top-6 right-6 z-[100] flex flex-col gap-3 pointer-events-none" style="min-width: 320px;">
            ...
            <div x-show="toast.visible" x-transition:enter="toast-enter" x-transition:leave="toast-leave"
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-white"
                        x-text="toast.type === 'success' ? 'Berhasil' : (toast.type === 'error' ? 'Kesalahan' : 'Peringatan')">
                    </p>
            ...
            Alpine.data('globalShortcuts', () => ({
                ...
                init() {
                    window.addEventListener('keydown', (e) => {
                        ...
                        const isInput = activeEl.tagName === 'INPUT' || activeEl.tagName ===
                            'TEXTAREA' || activeEl.tagName === 'SELECT' || activeEl
                            .isContentEditable;
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('realTimeClock', () => ({
                ...
                updateTime() {
                    ...
                    const timePart = new Intl.DateTimeFormat('id-ID', timeOptions).format(now);
                    this.currentTime = `${datePart} | ${timePart} WIB`;
                }
            }));
        });
    </script>
